...
class Dataset(data.Dataset):
    # def __init__(self, data_root, split='train', input_ratio=1, val_num=1, use_cache=False):
    def __init__(self, **kwargs):
        ...
        self.split = split
        input_ratio, val_num, use_cache = kwargs['input_ratio'], cfg.task_arg.val_num, cfg.task_arg.use_cache
        assert input_ratio >= 1, 'image can only be downsampled, please set img_downscale>=1!'
        self.input_ratio = input_ratio
        if split == 'val': # image downscale=1 will cause OOM in val mode
            self.input_ratio = max(2, self.input_ratio)
    ...
    def read_meta(self):
        ...
        if self.use_cache:
            with open(os.path.join(self.data_root, f'cache/Ks{self.input_ratio}.pkl'), 'rb') as f:
            ...
            for id_ in self.img_ids:
                ...
                img_w_, img_h_ = img_w//self.input_ratio, img_h//self.input_ratio
            ...
            if self.use_cache:
                ...
                for id_ in self.img_ids_train:
                    ...
                    if self.input_ratio > 1:
                        img_w = img_w//self.input_ratio
                        img_h = img_h//self.input_ratio
