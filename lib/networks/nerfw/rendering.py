...
def sample_pdf(bins, weights, N_importance, det=False, eps=1e-5):
    ...
    if det:
        u = torch.linspace(0, 1, N_importance)
        u = u.expand(N_rays, N_importance)
    else:
        u = torch.rand(N_rays, N_importance)
    u = u.contiguous().cuda()
...
#             noise = torch.randn_like(static_sigmas) * noise_std
    ...
    # Sample depth points
    # z_steps = torch.linspace(0., 1., steps=N_samples).to(near)
    z_steps = torch.linspace(0., 1., steps=N_samples).cuda()
    ...
    # xyz_coarse = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[
    ...
    if N_importance > 0: # sample points for fine model
        ...
        # xyz_fine = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[
